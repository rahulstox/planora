    ...
    const fetchContributors = async () => {
      try {
        const res = await fetch(
          "https://api.github.com/repos/Adarsh-Chaubey03/planora/contributors"
        );
          ...
          data.map(async (contributor) => {
            try {
              const prRes = await fetch(
                `https://api.github.com/search/issues?q=type:pr+repo:Adarsh-Chaubey03/planora+author:${contributor.login}`
              );
              ...
              const mergedPRs = await Promise.all(
                allPRs.map(async (pr) => {
                  ...
                  const prDetailsRes = await fetch(
                    `https://api.github.com/repos/Adarsh-Chaubey03/planora/pulls/${prNumber}`
                  );
